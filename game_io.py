...
class EventPlayer:
    ...
    async def move(self, map:Map):
        prev_walls = {
            (self.row + i, self.col + j)
            for i, j in DIRECTIONS
            if chr(map.protomap[self.row + i][self.col + j]) 
                    in Map.WALL_CHAR
                or chr(map.protomap[self.row+i][self.col+j])
                    in Map.BOUND_CHAR
        }
        ...
        while True:
            ...
            n_x += (
                math.cos(rad) * self.velocity * self.throttle[2]
            )
            n_y -= (
                math.sin(rad) * self.velocity * self.throttle[2]
            )
            n_x += (
    ...
    def set_button(self, button: Button, state: bool):
        ...
        logger.debug(
            f"is_button_held({str(button)}) -> "
            f"{bool(self.held_buttons & button)}"
        )
...
def user_event_player(**kwargs):
    global _user_event_player
    if _user_event_player is None:
        _user_event_player = EventPlayer(**kwargs)
